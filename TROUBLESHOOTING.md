# Troubleshooting & Recovery Guide — Loopix IDE

## Common Issues & Recovery

1. **Permission Approval Blocked**: Verify session state mode (`Strict`, `Assisted`, `Autonomous`). Respond via `ApprovalCard` UI.
2. **Session Interrupted / Crashed**: Loopix automatically recovers session state from persistent snapshots via `SessionRecoveryManager`.
